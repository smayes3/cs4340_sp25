...
import type { Info, Packet, Pie, Architecture, GitGraph, Radar, Activity } from './index.js';

export type DiagramAST = Info | Packet | Pie | Architecture | GitGraph | Radar | Activity;
...
const initializers = {
  ...
  activity: async () => {
    const { createActivityServices } = await import('./language/activity/index.js');
    const parser = createActivityServices().Activity.parser.LangiumParser;
    parsers.radar = parser;
  },
} as const;
...
export async function parse(diagramType: 'activity', text: string): Promise<Activity>;
